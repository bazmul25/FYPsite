...
{	
	 ...
	 */
	(lib.ExamplePointTracking = function(cameraResolution, brfResolution, brfRoi, faceDetectionRoi, screenRect, maskContainer, webcamInput)
	{
		var _this = this;
		...
		_super.constructor.call(this, cameraResolution, brfResolution, brfRoi, faceDetectionRoi, screenRect, maskContainer, webcamInput);
		...
		_this.updateGUI = function()
		{
			...
			if(_this._brfManager.state == lib.BRFState.POINT_TRACKING)
			{
				...
				if(points.length == 2)
				{
					var centerX = (points[1].x + points[0].x) * 0.5;
					var centerY = (points[1].y + points[0].y) * 0.5;

					lib.DrawingUtils.drawPoint(_this._draw, new lib.Point(centerX, centerY), 30);
					//console.log(centerX);

					/*var theExerciseArray = [];
					var img = document.createElement("img");

					if(theExerciseArray.length >= 1)
					{
						console.log("working");
						/*for	(i = 0; i < theExerciseArray.length; i++)
						{
							theExerciseArray[i] > _stage;
						}*/
					...
					{
						img.src = "css/images/exerciseOne.png";
						img.height = "720";
						img.width = "1280";
						theExerciseArray.push(img);

						document.getElementById('eyeExerciseImages').innerHTML = theExerciseArray;*/
					//}
				}
